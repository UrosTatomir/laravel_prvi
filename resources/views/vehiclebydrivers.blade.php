...
<head>
    ...
    <title>Vehicle by Drivers</title>
    ...
    <nav class="navbar fixed-top navbar-expand-lg  navbar-dark bg-dark ">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <div class="links">
                ...
                <a href="{{url('/choose_driverbyvehicle')}}">DriverByVehicles</a>
    ...
    <div class="container-fluid">
        <div class="container">
            <div class="container mt-5 p-5 col-8">
                ...
                //select * from vehicle
                $all_vehicle_by_driver = DB::table('vehicle')
                    ->join('vehicledrivers', 'vehicle.id_vehicle', '=', 'vehicledrivers.id_vehicle')
                    ->join('drivers', 'drivers.id_driver', '=', 'vehicledrivers.id_driver')
                    ->select('drivers.*', 'vehicledrivers.*', 'vehicle.*')
                    ->where('vehicle.id_vehicle', $id_vehicle)
                    ->get();
                //  $GETALLDRIVERSBYCAR = "SELECT vozaci.*,vozilavozaci.*,vozila.* FROM vozila JOIN vozilavozaci ON vozila.idvozila=vozilavozaci.idvozila JOIN vozaci ON vozaci.idvozaca=vozilavozaci.idvozaca WHERE vozila.idvozila=?";
                // dd($all_vehicle_by_driver);
                ?>
                <table class="table table-dark text-center table-sm">
                    ...
                    <tbody>
                        @foreach($all_vehicle_by_driver as $p)
                        <tr>
                            <td>{{$p->name}}</td>
                            <td>{{$p->surname}}</td>
                            <td>{{$p->age}}</td>
